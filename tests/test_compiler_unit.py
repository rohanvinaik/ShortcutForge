...
from shortcuts_compiler import (
    Shortcut, actions, ActionHandle, ref_variable, ref_extension_input,
    wrap_token_string, wrap_token_attachment, wrap_conditional_input,
    CONDITION_MAP, _resolve_identifier, _load_catalog
)
...
def test_bool_before_int():
    ...
    assert type_map["flag"] == 6, f"True should be type 6 (bool), got {type_map['flag']}"
    assert type_map["disabled"] == 6, f"False should be type 6 (bool), got {type_map['disabled']}"
    assert type_map["count"] == 3, f"int should be type 3 (number), got {type_map['count']}"
    assert type_map["rate"] == 3, f"float should be type 3 (number), got {type_map['rate']}"
    assert type_map["name"] == 0, f"str should be type 0 (string), got {type_map['name']}"
...
def test_resolve_full_identifier():
    assert _resolve_identifier("is.workflow.actions.gettext") == "is.workflow.actions.gettext"
...
def test_if_block_structure():
    s = Shortcut("Test")
    handle = s.add(actions.make("gettext",
        WFTextActionText=actions.build_token_string("hello")))
    with s.if_block(handle, condition="has_any_value"):
        s.add(actions.make("comment", WFCommentActionText="inside if"))

    conditionals = [a for a in s.actions
                    if "conditional" in a.get("WFWorkflowActionIdentifier", "")]
    assert len(conditionals) == 2, f"Expected 2 conditionals (start+end), got {len(conditionals)}"
...
def test_if_else_block_structure():
    ...
    conditionals = [a for a in s.actions
                    if "conditional" in a.get("WFWorkflowActionIdentifier", "")]
    assert len(conditionals) == 3, f"Expected 3 conditionals (start+else+end), got {len(conditionals)}"
...
def test_menu_block_structure():
    ...
    menus = [a for a in s.actions
             if "choosefrommenu" in a.get("WFWorkflowActionIdentifier", "")]
    assert len(menus) == 5, f"Expected 5 menu actions (start+3 cases+end), got {len(menus)}"
...
def test_nested_control_flow():
    ...
    with s.repeat_each_block(items):
        with s.menu_block("Pick:", ["X", "Y"]) as cases:
            cases["X"]()
            handle = s.add(actions.make("gettext",
                WFTextActionText=actions.build_token_string("test")))
            with s.if_block(handle, condition="has_any_value"):
    ...
    assert not crash_warnings, f"Nested control flow produced crash warnings: {crash_warnings}"
...
def test_validate_ordering():
    ...
    s.actions.append({
        "WFWorkflowActionIdentifier": "is.workflow.actions.conditional",
        "WFWorkflowActionParameters": {"GroupingIdentifier": gid, "WFControlFlowMode": 0,
                                       "WFCondition": 100},
    })
    warnings = s.validate()
    assert any("before start" in w for w in warnings), f"Expected ordering warning, got: {warnings}"
...
def test_validate_interleaving():
    ...
    s.actions.extend([
        {"WFWorkflowActionIdentifier": "is.workflow.actions.conditional",
         "WFWorkflowActionParameters": {"GroupingIdentifier": gid_a, "WFControlFlowMode": 0, "WFCondition": 100}},
        {"WFWorkflowActionIdentifier": "is.workflow.actions.conditional",
         "WFWorkflowActionParameters": {"GroupingIdentifier": gid_b, "WFControlFlowMode": 0, "WFCondition": 100}},
        {"WFWorkflowActionIdentifier": "is.workflow.actions.conditional",
         "WFWorkflowActionParameters": {"GroupingIdentifier": gid_a, "WFControlFlowMode": 2}},
        {"WFWorkflowActionIdentifier": "is.workflow.actions.conditional",
         "WFWorkflowActionParameters": {"GroupingIdentifier": gid_b, "WFControlFlowMode": 2}},
    ])
    warnings = s.validate()
    assert any("interleaved" in w.lower() or "overlap" in w.lower() for w in warnings), \
...
def test_validate_mode1_outside_range():
    ...
    s.actions.extend([
        ...
        {"WFWorkflowActionIdentifier": "is.workflow.actions.conditional",
         "WFWorkflowActionParameters": {"GroupingIdentifier": gid, "WFControlFlowMode": 0, "WFCondition": 100}},
    ...
    s.actions.extend([
        {"WFWorkflowActionIdentifier": "is.workflow.actions.choosefrommenu",
         "WFWorkflowActionParameters": {
             ...
             "WFMenuPrompt": "Pick:", "WFMenuItems": ["A", "B", "C"]}},
        {"WFWorkflowActionIdentifier": "is.workflow.actions.choosefrommenu",
         "WFWorkflowActionParameters": {"GroupingIdentifier": gid, "WFControlFlowMode": 1,
                                        "WFMenuItemTitle": "A"}},
        {"WFWorkflowActionIdentifier": "is.workflow.actions.choosefrommenu",
...
def test_condition_map_coverage():
    ...
    for name in ["has_any_value", "does_not_have_any_value", "equals_number",
                 "is_greater_than", "is_less_than", "equals_string",
                 "not_equal_string", "contains", "does_not_contain", "is_before",
                 "==", "!=", ">", "<"]:
        assert name in CONDITION_MAP, f"Missing condition: {name}"
        assert isinstance(CONDITION_MAP[name], int), f"Condition {name} should map to int"
