...
def test_hk_unit_match_ok():
    ...
    ir = _make_ir([
        _make_action("health.quantity.log", {
...
def test_hk_unit_mismatch():
    ...
    layer.validate(ir, result)
    unit_warnings = [w for w in result.warnings if w.category == "hk_unit_mismatch"]
    assert len(unit_warnings) == 1, f"Expected 1 unit mismatch warning, got {len(unit_warnings)}"
...
def test_downloadurl_with_if_ok():
    ...
    layer.validate(ir, result)
    err_warnings = [w for w in result.warnings if w.category == "missing_error_handling"]
...
def test_downloadurl_in_nested_block():
    ...
    ir = _make_ir([
        IfBlock(
            ...
            compare_value=None,
            then_body=[
                _make_action("downloadurl", {"WFHTTPMethod": StringValue("GET")}, line=3),
...
def test_no_unreachable_normal_flow():
    ...
    ir = _make_ir([
        _make_action("comment", {"WFCommentActionText": StringValue("step 1")}, line=1),
        _make_action("showresult", {"Text": StringValue("done")}, line=2),
    ])
    result = ValidationResult()
    layer = DomainValidationLayer()
...
def test_no_domain_data_no_hk_warnings():
    ...
    layer.validate(ir, result)
    hk_warnings = [w for w in result.warnings
...
run_test("no_domain_data_still_checks_error_handling", test_no_domain_data_still_checks_error_handling)
...
'''
    ...
    err_warnings = [w for w in result.warnings if w.category == "missing_error_handling"]
    assert len(err_warnings) >= 1, f"Expected missing_error_handling warning, got categories: {[w.category for w in result.warnings]}"
