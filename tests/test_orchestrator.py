...
def test_classify_unknown_action():
    assert (
        classify_failure(None, ["Unknown action: 'foo'"], None, False)
        == FailureType.UNKNOWN_ACTION
    )


def test_classify_unknown_action_lowercase():
    assert (
        classify_failure(None, ["unknown_action: something"], None, False)
        == FailureType.UNKNOWN_ACTION
    )
...
def test_classify_bad_params():
    assert (
        classify_failure(None, ["bad parameter value"], None, False)
        == FailureType.BAD_PARAMS
    )
...
def test_budget_complex_signals():
    b = estimate_budget(
        "Create a shortcut with a menu and conditional check for each item in a list"
    )
...
def test_budget_complex_long_prompt():
    ...
    assert b.complexity == "complex", (
        f"Expected complex for long prompt, got {b.complexity}"
    )
...
def test_budget_very_complex_signals():
    ...
    b = estimate_budget(
        "Create a shortcut that uses a menu, loops through each item, checks conditionally, and compares values in a list"
    )
    assert b.complexity == "very_complex", f"Expected very_complex, got {b.complexity}"
    assert b.max_tokens == 4096
...
def test_budget_very_complex_long_prompt():
    ...
    assert b.complexity == "very_complex", (
        f"Expected very_complex for very long prompt, got {b.complexity}"
    )
    assert b.max_tokens == 4096
...
def test_budget_very_complex_char_len():
    ...
    assert b.complexity == "very_complex", (
        f"Expected very_complex for long char prompt, got {b.complexity}"
    )
    assert b.max_tokens == 4096
...
for name, fn in [
    ...
    (
        "overflow_not_detected_with_endshortcut",
        test_overflow_not_detected_with_endshortcut,
    ),
