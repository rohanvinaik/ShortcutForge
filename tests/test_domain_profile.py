...
def test_select_health_profile():
    ...
    profile = mgr.select_profile(
        "Log my caffeine intake and vitamin supplements to Apple Health"
    )
    assert profile.profile_id == "health_logger", (
        f"Expected health_logger, got {profile.profile_id}"
    )
...
def test_select_api_profile():
    ...
    mgr = DomainProfileManager()
    profile = mgr.select_profile(
        "Fetch JSON from the REST API endpoint and parse the response"
    )
    assert profile.profile_id == "api_workflow", (
        f"Expected api_workflow, got {profile.profile_id}"
    )
...
def test_select_general_fallback():
    ...
    assert profile.profile_id == "general", (
        f"Expected general, got {profile.profile_id}"
    )
