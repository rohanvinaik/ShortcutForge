...
from shortcuts_compiler import Shortcut, actions, ref_extension_input
...
with sc.if_else_block(ref_extension_input(), condition="has_any_value") as otherwise:
    ...
    chosen_show_raw = sc.add(
        actions.make(
            "choosefromlist", WFChooseFromListActionPrompt="Which show?", WFInput=pods
        )
    )
    sc.add(
        actions.make(
            "setvariable", WFVariableName="Chosen Show", WFInput=chosen_show_raw
        )
    )
    episodes = sc.add(actions.make("getepisodesforpodcast", WFInput=chosen_show_raw))
    chosen_ep_raw = sc.add(
        actions.make(
            "choosefromlist",
            WFChooseFromListActionPrompt="Which episode?",
            WFInput=episodes,
        )
    )
    sc.add(
        actions.make(
            "setvariable", WFVariableName="Chosen Episode", WFInput=chosen_ep_raw
        )
    )
    store_url = sc.add(
        actions.make(
...
split1 = sc.add(
    actions.make(
        ...
        WFTextCustomSeparator="id",
        **{"Show-text": True},
    )
)
last_item = sc.add(
    actions.make("getitemfromlist", WFInput=split1, WFItemSpecifier="Last Item")
)
split2 = sc.add(
    actions.make(
        "text.split",
...
search_result = sc.add(
    actions.make("searchpodcasts", WFAttribute="Product ID", WFSearchTerm=first_item)
)
show_url_val = sc.add(
    actions.make(
        "properties.podcastshow",
        WFContentItemPropertyName="Store URL",
        WFInput=search_result,
    )
)
...
with sc.if_else_block(store_url, condition="has_any_value") as otherwise2:
    ...
    title_split = sc.add(
        actions.make(
            ...
            text=article_title,
            WFTextSeparator="Custom",
            WFTextCustomSeparator=" on Apple Podcasts",
            **{"Show-text": True},
        )
    )
    clean_title = sc.add(actions.make("getitemfromlist", WFInput=title_split))
    sc.add(
        actions.make("setvariable", WFVariableName="Full Title", WFInput=clean_title)
    )
    ...
    sc.add(
        actions.make("setvariable", WFVariableName="Episode URL", WFInput=ep_store_url)
    )
...
timestamp = sc.add(
    actions.make(
        "ask",
        WFAskActionPrompt="What\u2019s the timestamp?",
        CustomOutputName="Timestamp",
    )
)
quote = sc.add(
    actions.make(
        "ask", WFAskActionPrompt="What\u2019s the quote?", CustomOutputName="Quote"
    )
)
speaker = sc.add(
    actions.make(
        "ask",
        WFAskActionPrompt="Who\u2019s the speaker?",
        CustomOutputName="Speaker Name",
    )
)
...
with sc.menu_block("Where do you want to save it?", ["Day One", "Notes"]) as cases:
    ...
    sc.add(
        actions.make(
            ...
            EntryJournal="Personal",
            EntryTags=speaker,
            EntryText=entry_text,
            imageClipboard=quote,
        )
    )
...
filepath = sc.save(
    os.path.join(tempfile.gettempdir(), "Quote_of_the_Day_reconstructed.shortcut")
)
