...
from dsl_validator import get_validator, validate_ir
...
for name, fn in [
    ...
    (
        "tier3_known_vendor_permissive_warning",
        test_tier3_known_vendor_permissive_warning,
    ),
]:
    run_test(name, fn)
...
def test_tier3_compiler_risk_warning():
    ...
    assert len(risk_warns) >= 1, (
        f"Expected compiler_risk warning, got categories: {[w.category for w in perm.warnings]}"
    )
