...
def test_var_unused():
    ...
    ir = _make_ir("test", [
        ...
        ActionStatement("showresult", {"WFText": StringValue("done")}, line_number=2),
    ])
    report = SimulationHarness().analyze(ir)
    flow = report.findings_by_category(FindingCategory.VARIABLE_FLOW)
...
def test_var_loop_scoped():
    ...
    ir = _make_ir("test", [
        ...
        ActionStatement("showresult", {"WFText": VarRef("loop_var")}, line_number=5),
    ])
    report = SimulationHarness().analyze(ir)
    flow = report.findings_by_category(FindingCategory.VARIABLE_FLOW)
...
def test_loop_large_count():
    ...
    ir = _make_ir("test", [
        RepeatBlock(count=NumberValue(5000), body=[
            ActionStatement("showresult", {"WFText": StringValue("hi")}, line_number=2),
        ], line_number=1),
    ])
    report = SimulationHarness().analyze(ir)
...
def test_loop_zero_count():
    ...
    ir = _make_ir("test", [
        RepeatBlock(count=NumberValue(0), body=[
            ActionStatement("showresult", {"WFText": StringValue("hi")}, line_number=2),
        ], line_number=1),
    ])
    report = SimulationHarness().analyze(ir)
...
def test_loop_normal_count():
    ...
    ir = _make_ir("test", [
        RepeatBlock(count=NumberValue(10), body=[
            ActionStatement("showresult", {"WFText": StringValue("hi")}, line_number=2),
        ], line_number=1),
    ])
    report = SimulationHarness().analyze(ir)
...
def test_menu_duplicate_labels():
    ...
    ir = _make_ir("test", [
        MenuBlock(
            prompt="Pick",
            cases=[
                MenuCase("Option A", [ActionStatement("showresult", {"WFText": StringValue("a")}, line_number=3)]),
                MenuCase("Option A", [ActionStatement("showresult", {"WFText": StringValue("b")}, line_number=5)]),
            ],
            line_number=1,
        ),
    ])
    ...
    assert len(dup) >= 1, f"Expected duplicate label warning, got: {[f.message for f in menu]}"
...
def test_menu_empty_case():
    ...
    assert len(empty) >= 1, f"Expected empty case info, got: {[f.message for f in menu]}"
...
def test_menu_single_case():
    ...
    ir = _make_ir("test", [
        MenuBlock(
            prompt="Pick",
            cases=[
                MenuCase("Only Option", [ActionStatement("showresult", {"WFText": StringValue("a")}, line_number=3)]),
            ],
            line_number=1,
        ),
    ])
    ...
    assert len(single) >= 1, f"Expected single-case info, got: {[f.message for f in menu]}"
...
def test_dead_code_after_exit():
    ...
    ir = _make_ir("test", [
        ...
        ActionStatement("showresult", {"WFText": StringValue("unreachable")}, line_number=2),
    ])
    report = SimulationHarness().analyze(ir)
...
def test_no_dead_code_normal():
    ...
    ir = _make_ir("test", [
        ActionStatement("showresult", {"WFText": StringValue("one")}, line_number=1),
        ActionStatement("showresult", {"WFText": StringValue("two")}, line_number=2),
    ])
    report = SimulationHarness().analyze(ir)
    dead = report.findings_by_category(FindingCategory.DEAD_CODE)
...
def test_dead_code_comments_ok():
    ...
    report = SimulationHarness().analyze(ir)
    dead = report.findings_by_category(FindingCategory.DEAD_CODE)
    assert len(dead) == 0, f"Expected no dead code warning for comments, got {len(dead)}"
...
def test_api_malformed_url():
    ...
    assert len(url_warns) >= 1, f"Expected malformed URL warning, got: {[f.message for f in api]}"
...
def test_api_valid_url():
    ...
    api = report.findings_by_category(FindingCategory.API_VALIDATION)
    url_warns = [f for f in api if "malformed" in f.message]
    assert len(url_warns) == 0, f"Expected no URL warnings, got: {[f.message for f in api]}"
...
def test_api_invalid_method():
    ...
    assert len(method_warns) >= 1, f"Expected invalid method warning, got: {[f.message for f in api]}"
...
def test_api_post_without_body():
    ...
    assert len(body_warns) >= 1, f"Expected POST-without-body info, got: {[f.message for f in api]}"
...
def test_type_mismatch_prev():
    ...
    ir = _make_ir("test", [
        ...
        ActionStatement("detect.dictionary", {
            "WFInput": HandleRef("prev"),
        }, line_number=2),  # expects data
    ])
    report = SimulationHarness().analyze(ir)
...
def test_report_properties():
    ...
    report = SimulationReport(findings=[
...
def test_selective_analysis():
    ...
    ir = _make_ir("test", [
        ...
        ActionStatement("showresult", {"WFText": StringValue("dead")}, line_number=5),
    ])
...
def test_clean_ir_minimal_findings():
    ...
    ir = _make_ir("Clean Shortcut", [
        SetVariable("name", StringValue("World"), line_number=1),
        ActionStatement("showresult", {
            "WFText": InterpolatedString((StringValue("Hello, "), VarRef("name"))),
        }, line_number=2),
    ])
    report = SimulationHarness().analyze(ir)
    errors_warnings = [
