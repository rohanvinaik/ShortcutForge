...
def main():
    ...
    parser.add_argument("--train-in", type=Path,
    ...
    parser.add_argument("--in-domain-n", type=int, default=500,
                       help="Number of in-domain samples to include")
    parser.add_argument("--ood-n", type=int, default=500,
                       help="Number of OOD samples to generate")
    ...
    from research.src.contracts import OODPrompt
    ...
    # ══════════════════════════════════════════════════════════════════
    # Generate OOD prompts from templates with variation
    # ══════════════════════════════════════════════════════════════════
    print("=== Generating OOD prompts ===")
    all_ood_prompts: list[OODPrompt] = []

    for category, templates in OOD_TEMPLATES.items():
        category_prompts: list[OODPrompt] = []
        for template in templates:
            for prefix in PREFIXES:
                for suffix in SUFFIXES:
                    # Skip the identity (no prefix, no suffix) for some variation
                    # but keep it too for the base template
                    ...
                    if prefix and prompt_text:
                        # The prefix already has the capital, lowercase the template start
                        prompt_text = prefix + template[0].lower() + template[1:] + suffix
                        prompt_text = prompt_text.strip()

                    category_prompts.append(OODPrompt(
                        prompt=prompt_text,
                        label="ood",
                        category=category,
                        source="synthetic",
    ...
    # Sample --ood-n from the pool
    if len(all_ood_prompts) > args.ood_n:
        ood_sample = random.sample(all_ood_prompts, args.ood_n)
    else:
        ood_sample = all_ood_prompts
    print(f"  Sampled {len(ood_sample)} OOD prompts")

    # ══════════════════════════════════════════════════════════════════
    # Load in-domain prompts from training data
    # ══════════════════════════════════════════════════════════════════
    print("\n=== Loading in-domain prompts ===")
    in_domain_prompts: list[OODPrompt] = []

    if not args.train_in.exists():
        print(f"  WARNING: {args.train_in} not found, skipping in-domain prompts")
    else:
        ...
        with open(args.train_in) as f:
            for line in f:
                ...
                try:
                    ...
                    # Extract user message (messages[1] is user role)
                    for msg in messages:
        ...
        # Sample --in-domain-n
        if len(raw_prompts) > args.in_domain_n:
            sampled = random.sample(raw_prompts, args.in_domain_n)
        else:
            sampled = raw_prompts

        for prompt_text in sampled:
            in_domain_prompts.append(OODPrompt(
                prompt=prompt_text,
                label="in_domain",
    ...
    # ══════════════════════════════════════════════════════════════════
    # Optional seed file with manual OOD examples
    # ══════════════════════════════════════════════════════════════════
    seed_entries: list[OODPrompt] = []
    if args.seed_file and args.seed_file.exists():
        print(f"\n=== Loading seed file: {args.seed_file} ===")
    ...
    # ══════════════════════════════════════════════════════════════════
    combined: list[OODPrompt] = ood_sample + in_domain_prompts + seed_entries
    ...
    if args.dry_run:
        ...
        print(f"\n  Wrote {len(combined)} entries to {args.out}")
