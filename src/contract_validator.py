...
def _extract_var_refs_from_stmt(stmt: Statement) -> set[str]:
    ...
    if isinstance(stmt, ActionStatement):
        ...
        elif hasattr(stmt.count, '__class__') and isinstance(stmt.count, IRValue.__args__ if hasattr(IRValue, '__args__') else ()):
...
class ContractValidator:
    ...
    def _rule_api_missing_error_check(self, flat: list[FlatEntry]) -> list[ContractFinding]:
    ...
    def _rule_api_missing_content_type(self, flat: list[FlatEntry]) -> list[ContractFinding]:
    ...
    def _rule_api_json_parse_after_fetch(self, flat: list[FlatEntry]) -> list[ContractFinding]:
    ...
    def _rule_api_url_not_variable(self, flat: list[FlatEntry]) -> list[ContractFinding]:
        ...
        def walk(statements: list[Statement]) -> None:
            for stmt in statements:
                if isinstance(stmt, ForeachBlock):
                    non_comment_body = [s for s in stmt.body if not isinstance(s, Comment)]
    ...
    def _rule_dictionary_unused_keys(self, flat: list[FlatEntry]) -> list[ContractFinding]:
    ...
    def _collect_all_refs(self, stmts: list[Statement], refs: set[str]) -> None:
        ...
        for stmt in stmts:
            if isinstance(stmt, ActionStatement):
                ...
                # The value side references variables (not the var_name being set)
                refs |= _extract_var_refs_from_value(stmt.value)
            elif isinstance(stmt, IfBlock):
                if isinstance(stmt.target, VarRef):
                    refs.add(stmt.target.name)
                if stmt.compare_value:
                    refs |= _extract_var_refs_from_value(stmt.compare_value)
                self._collect_all_refs(stmt.then_body, refs)
                if stmt.else_body:
                    self._collect_all_refs(stmt.else_body, refs)
            elif isinstance(stmt, MenuBlock):
                for case in stmt.cases:
                    self._collect_all_refs(case.body, refs)
            elif isinstance(stmt, RepeatBlock):
                if isinstance(stmt.count, VarRef):
                    refs.add(stmt.count.name)
                self._collect_all_refs(stmt.body, refs)
            elif isinstance(stmt, ForeachBlock):
                if isinstance(stmt.collection, VarRef):
                    refs.add(stmt.collection.name)
                self._collect_all_refs(stmt.body, refs)
        ...
        def walk(statements: list[Statement], in_loop: bool) -> None:
            for stmt in statements:
                if isinstance(stmt, ActionStatement):
                    ...
                    walk(stmt.then_body, in_loop)
                    if stmt.else_body:
                        walk(stmt.else_body, in_loop)
                elif isinstance(stmt, MenuBlock):
                    for case in stmt.cases:
                        walk(case.body, in_loop)

        walk(stmts, False)
        return findings

    def _rule_menu_duplicate_labels(self, stmts: list[Statement]) -> list[ContractFinding]:
    ...
    def _rule_use_before_set(self, stmts: list[Statement]) -> list[ContractFinding]:
        ...
        def walk(statements: list[Statement], local_defined: set[str]) -> None:
            for stmt in statements:
                ...
                # Recurse into control flow (sharing the same scope)
                if isinstance(stmt, IfBlock):
                    walk(stmt.then_body, set(local_defined))
                    if stmt.else_body:
                        walk(stmt.else_body, set(local_defined))
                elif isinstance(stmt, MenuBlock):
                    for case in stmt.cases:
                        walk(case.body, set(local_defined))
                elif isinstance(stmt, RepeatBlock):
                    walk(stmt.body, set(local_defined))
                elif isinstance(stmt, ForeachBlock):
                    walk(stmt.body, set(local_defined))
    ...
    def _rule_shadow_in_loop(self, stmts: list[Statement]) -> list[ContractFinding]:
        ...
        def walk(statements: list[Statement], outer_vars: set[str], in_loop: bool) -> None:
            for stmt in statements:
                if isinstance(stmt, SetVariable):
                    if in_loop and stmt.var_name in outer_vars:
                        findings.append(ContractFinding(
                            ...
                            line=stmt.line_number,
                            suggestion=f"Use a different variable name inside the loop",
                        ))
                    # Add to outer_vars for subsequent statements
                    outer_vars.add(stmt.var_name)

                if isinstance(stmt, ForeachBlock):
                    walk(stmt.body, set(outer_vars), True)
                elif isinstance(stmt, RepeatBlock):
                    walk(stmt.body, set(outer_vars), True)
                elif isinstance(stmt, IfBlock):
                    walk(stmt.then_body, set(outer_vars), in_loop)
                    if stmt.else_body:
                        walk(stmt.else_body, set(outer_vars), in_loop)
                elif isinstance(stmt, MenuBlock):
                    for case in stmt.cases:
                        walk(case.body, set(outer_vars), in_loop)

        walk(stmts, set(), False)
        return findings

    def _rule_dead_code_after_exit(self, stmts: list[Statement]) -> list[ContractFinding]:
