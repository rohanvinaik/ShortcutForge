...
url = s.add(
    actions.make(
        "url",
        WFURLActionURL=wrap_token_string(
            "https://api.example.com/log?key=", ref_extension_input()
        ),
    )
)
...
response_var = s.add(
    actions.make("getvariable", WFVariable=ref_variable("ResponseData"))
)
status = s.add(
    actions.make("getvalueforkey", WFDictionaryKey="status", WFInput=response_var)
)
...
with s.if_else_block(status, condition="has_any_value") as otherwise:
    ...
    message = s.add(
        actions.make("getvalueforkey", WFDictionaryKey="message", WFInput=msg_var)
    )
    s.add(
        actions.make(
            "notification",
