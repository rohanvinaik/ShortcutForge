...
def main():
    ...
    print(
        f"   Shortcuts with if/else (conditional): {len(shortcuts_by_type['conditional'])}"
    )
    ...
    print(
        f"   Shortcuts with repeat with each: {len(shortcuts_by_type['repeat_each'])}"
    )
    print()
    ...
    if mode_0_total == 0 and mode_2_total == 0:
        print(
            "   NOTE: No traditional block start/end modes found (all actions use mode None)"
        )
        print("   This suggests a different control flow structure than expected.")
        print(f"   Total actions with mode None or missing: {other_modes}")
    elif mode_0_total == mode_2_total:
        print(
            f"   ✓ VALID: Block starts (mode 0) = Block ends (mode 2) = {mode_0_total}"
        )
        print("   All blocks appear to be properly closed!")
    else:
        print(
            f"   ✗ MISMATCH: Block starts (mode 0) = {mode_0_total}, Block ends (mode 2) = {mode_2_total}"
        )
        print(f"   Difference: {abs(mode_0_total - mode_2_total)} blocks")
        if mode_0_total > mode_2_total:
            print(
                "   WARNING: More block starts than ends - some blocks may not be closed!"
            )
        else:
            print(
                "   WARNING: More block ends than starts - structure may be malformed!"
            )
    print()
    ...
    if stats["unusual_modes"]:
        print("   Found unusual mode values:")
    ...
    if not stats["unusual_modes"] and none_count == 0:
        print("   ✓ Only modes 0, 1, and 2 used")
    print()
    ...
    if missing > 0 and missing <= 20:
        ...
        for i, item in enumerate(stats["missing_grouping"]):
            print(
                f"      {i + 1}. {item['file']} - {item['type']} (mode {item['mode']})"
            )
    elif missing > 20:
    ...
    if stats["unknown_control_flow_actions"]:
        print(
            f"   Found {len(stats['unknown_control_flow_actions'])} actions with WFControlFlowMode set"
        )
        print("   (These are actions not in our known control flow list):")
        print()
        ...
        print("   ✓ No unknown control flow actions found")
    print()
