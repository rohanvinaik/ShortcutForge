...
def main():
    ...
    with open(report_path, "w") as f:
        ...
        f.write(
            f"Shortcuts with control flow: {stats['shortcuts_with_control_flow']}\n"
        )
        f.write(
            f"Percentage with control flow: {stats['shortcuts_with_control_flow'] / stats['total_shortcuts'] * 100:.1f}%\n"
        )
        f.write("\n")
        ...
        for action_id in CONTROL_FLOW_ACTIONS:
            ...
            f.write(
                f"{action_type:20s}: {shortcut_count:3d} shortcuts using it, {action_count:4d} total actions\n"
            )
        f.write("\n")
        ...
        f.write("-" * 90 + "\n")
        f.write(
            f"{'Action Type':20s} {'Mode 0':>10s} {'Mode 1':>10s} {'Mode 2':>10s} {'Total':>10s}\n"
        )
        f.write("-" * 90 + "\n")
        for action_type in CONTROL_FLOW_ACTIONS.values():
            ...
            f.write(
                f"{action_type:20s} {mode_0:10d} {mode_1:10d} {mode_2:10d} {total:10d}\n"
            )
        ...
        f.write("-" * 90 + "\n")
        f.write(
            f"{'TOTAL':20s} {mode_0_total:10d} {mode_1_total:10d} {mode_2_total:10d} {grand_total:10d}\n"
        )
        f.write("\n")
        ...
        if mode_0_total == mode_2_total:
            ...
            if mode_0_total > mode_2_total:
                f.write(
                    f"WARNING: More block starts than ends - {mode_0_total - mode_2_total} unclosed blocks\n"
                )
            else:
                f.write(
                    "WARNING: More block ends than starts - malformed structure detected\n"
                )
        f.write("\n")
        ...
        if stats["block_mismatches"]:
            f.write(
                f"Found {len(stats['block_mismatches'])} shortcuts with mismatches:\n\n"
            )
            for item in stats["block_mismatches"]:
                diff = item["mode_0"] - item["mode_2"]
                if diff > 0:
                    f.write(
                        f"  {item['file']:50s} {item['mode_0']} starts, {item['mode_2']} ends (excess starts: {diff})\n"
                    )
                else:
                    f.write(
                        f"  {item['file']:50s} {item['mode_0']} starts, {item['mode_2']} ends (excess ends: {-diff})\n"
                    )
        else:
        ...
        coverage_pct = (
            ((total_control_flow - missing) / total_control_flow * 100)
        ...
        if missing > 0:
            ...
            f.write("First 20 actions missing GroupingIdentifier:\n")
            for i, item in enumerate(stats["missing_grouping"][:20]):
                f.write(
                    f"  {i + 1:2d}. {item['file']:50s} {item['type']:20s} (mode {item['mode']})\n"
                )
            if missing > 20:
        ...
        if stats["unknown_control_flow_actions"]:
            f.write(
                f"Found {len(stats['unknown_control_flow_actions'])} actions with WFControlFlowMode outside known control flow:\n\n"
            )
        ...
        f.write("-" * 90 + "\n")
        f.write(
            f"Total actions with 'flow' or 'control' in identifier: {sum(stats['flow_action_counts'].values())}\n\n"
        )
        f.write("Top 30 flow/control related actions by frequency:\n")
        f.write(f"{'Action ID':60s} {'Count':>10s}\n")
        f.write("-" * 90 + "\n")
        sorted_flow = sorted(
            stats["flow_action_counts"].items(), key=lambda x: x[1], reverse=True
        )
    ...
    print()
    print(
        f"1. Control Flow Coverage: {stats['shortcuts_with_control_flow']}/{stats['total_shortcuts']} shortcuts ({stats['shortcuts_with_control_flow'] / stats['total_shortcuts'] * 100:.1f}%)"
    )
    print()
    ...
    for action_id in CONTROL_FLOW_ACTIONS:
        ...
        action_count = stats["control_flow_action_counts"][action_type]
        print(
            f"   - {action_type}: {shortcut_count} shortcuts, {action_count} total actions"
        )
    print()
    print("3. Block Structure Validation:")
    if mode_0_total == mode_2_total:
        print(
            f"   ✓ VALID: {mode_0_total} block starts matched with {mode_2_total} block ends"
        )
    else:
        print(
            f"   ✗ MISMATCH: {mode_0_total} block starts vs {mode_2_total} block ends (diff: {abs(mode_0_total - mode_2_total)})"
        )
    print()
    print(
        "4. GroupingIdentifier Coverage: 100%"
        if missing == 0
        else f"   GroupingIdentifier: {100 * coverage_pct:.1f}%"
    )
    print()
    ...
    if stats["block_mismatches"]:
        print(
            f"6. ATTENTION: {len(stats['block_mismatches'])} shortcuts have block start/end mismatches"
        )
    else:
